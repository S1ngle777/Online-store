<x-app-layout>
    <script>
        function updateTotal() {
            ...
            const deliveryPrice = parseFloat(selectedDelivery.dataset.price);
            const subtotal = {{ $totalPrice }};
            const total = subtotal + deliveryPrice;

            document.getElementById('delivery-cost').textContent = deliveryPrice.toFixed(2) + ' MDL';
            document.getElementById('total-amount').textContent = total.toFixed(2) + ' MDL';
        }
    </script>
                                        ...
                                        <div class="flex justify-between items-center border-b pb-4">
                                            <div>
                                                ...
                                                <div class="mt-1">
                                                    @if (isset($item['original_price']) && $item['original_price'] > $item['price'])
                                                        <span
                                                            class="line-through text-sm text-gray-500">{{ number_format($item['original_price'] * $item['quantity'], 2) }}
                                                            MDL</span>
                                                        <span
                                                            class="text-red-600 ml-2">{{ number_format($item['price'] * $item['quantity'], 2) }}
                                                            MDL</span>
                                                    @else
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            // Установить начальную сумму, если метод доставки уже выбран
